...
class CIFAR101:
    def __init__(self,
        ...
        self.test_loader = torch.utils.data.DataLoader(
            self.test_dataset, batch_size=batch_size, shuffle=shuffle, **kwargs
        )
...
class CIFAR10C:
    def __init__(self, preprocess, corruption, severity=None, location=os.path.expanduser('~/data'), shuffle=False,batch_size=128, num_workers=16, classnames=None):

        test_data_root = os.path.join(location, "CIFAR-10-C")
        test_data_images = np.load(os.path.join(test_data_root, f"{corruption}.npy"), allow_pickle=True)
        test_data_labels = np.load(os.path.join(test_data_root, f"labels.npy"), allow_pickle=True)
        if isinstance(severity,list):
            indices = [slice((s-1)*10000, (s)*10000) for s in severity]
        else:
            indices = [slice(i,len(test_data_images),5) for i in range(5)]
        test_data_images = np.concatenate([test_data_images[indice] for indice in indices])
        test_data_labels = np.concatenate([test_data_labels[indice] for indice in indices])
